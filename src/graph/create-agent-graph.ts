...
import { routeAfterModel, routeAfterTools } from "./routers.js";
...
export function createAgentGraph<TOutput extends Record<string, unknown>>(options: {
  ...
  return new StateGraph(AgentGraphState)
    ...
    .addConditionalEdges(
      "call-model",
      (state) => routeAfterModel(state, structuredOutputToolName),
      ["execute-tools", "finalize", "record-protocol-error"],
    )
    .addConditionalEdges(
      "execute-tools",
